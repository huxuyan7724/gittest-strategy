...
class Database:
    ...
    def get_top_prescribed_item_percentage(self):
        """Return the percentage of the mostly prescribed item."""
        Temp_list=db.session.query(func.sum(PrescribingData.items).group_by(PrescribingData.BNF_code))
        return round((max(Temp_list)/sum(Temp_list))*100,2)